...
export function ProfileCard({
  ...
  return (
    ...
    >
      <div
        className={`absolute top-0 right-0 w-8 h-8 md:w-12 md:h-12 border-t-4 border-r-4 transition-colors duration-300 ${isHovered ? "border-red-500" : "border-black/20 dark:border-white/20"}`}
      ></div>

      <div className="flex flex-col h-full z-10 relative">
        <div
          className={`w-16 h-16 mb-8 flex items-center justify-center transition-colors duration-300 ${isHovered ? "text-red-500" : "text-black dark:text-white"}`}
        >
          {icon}
        </div>
        ...
        <div className="mt-auto pt-8 border-t-2 border-black/20 dark:border-white/20 flex items-center justify-between">
          <button
            className={`relative overflow-hidden px-8 py-4 font-space font-bold uppercase tracking-widest text-[10px] md:text-sm border-2 border-black dark:border-white transition-colors duration-300 w-full flex items-center justify-center gap-4 ${isHovered ? "bg-red-500 border-red-500 text-white" : "bg-transparent text-black dark:text-white"}`}
          >
