...
const downloadReceipt = asyncHandler(async (req: Request, res: Response) => {
  ...
  const safeRef = (purchase.purchase_reference as string)
    .slice(0, 16)
    .toUpperCase();
  const filename = `devdistro-receipt-${safeRef}.pdf`;

  res.setHeader("Content-Type", "application/pdf");
  res.setHeader("Content-Disposition", `attachment; filename="${filename}"`);
  doc.pipe(res);

  // Helper: draw the DevDistro "D" logo in two-tone (black top-left / red bottom-right)
  const drawDevDistroLogo = (lx: number, ly: number, size: number): void => {
    const s = size / 100;
  ...
  // ── HEADER ──
  drawDevDistroLogo(50, 46, 28); // 28×28pt logo

  doc
    .font("Helvetica-Bold")
    .fontSize(22)
    .fillColor("#000000")
    .text("DEVDISTRO", 84, 50);
  ...
  const terms = [
    "1. This receipt confirms that the above purchase was executed on the DevDistro platform and that payment was transferred via the Solana blockchain as indicated by the transaction signature above.",
    ...
    "5. DevDistro acts solely as a marketplace facilitator and is not liable for the quality, functionality, security, or fitness for purpose of any project sold through the platform.",
    "6. In the event of a dispute, buyers should contact DevDistro support with the Purchase Reference above. DevDistro will make reasonable efforts to assist but cannot guarantee resolution.",
    "7. This document serves as proof of purchase for the transaction described herein. It is generated automatically by DevDistro and is valid without a physical signature.",
  ];
  ...
  doc
    ...
    .fontSize(8)
    .fillColor("#888888")
    .text(`Generated by DevDistro · ${generatedAt}`, 50, y, {
      width: pageWidth,
      align: "center",
    });
